  ...
  return (
    <div className="space-y-6">
      {results.map((result) => (
        <Card
          key={result.id}
          className="overflow-hidden transition-all duration-200 hover:shadow-soft bg-white dark:bg-gray-800"
        >
          <CardContent className="p-6">
            ...
            <Link href={`/post/${result.id}`} className="group">
            ...
            <p className="mb-4 text-sm text-gray-600 dark:text-gray-300 line-clamp-2">{result.snippet}</p>

            <div className="text-sm text-gray-500 dark:text-gray-400">
              By <span className="font-medium text-gray-700 dark:text-gray-300">{result.author}</span>
            </div>
          </CardContent>
